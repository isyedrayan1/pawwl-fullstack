...
const main = async () => {
  const backup = JSON.parse(fs.readFileSync(backupPath, "utf8")) as CatalogBackup;

  const animalTypes = Array.from(new Set(backup.items.map((item) => item.animalType).filter((value): value is string => Boolean(value))));
  const categories = Array.from(new Set(backup.items.map((item) => item.category).filter(Boolean)));

  const animalTypeLookup = new Map<string, { id: string; key: string; name: string }>();
  const categoryLookup = new Map<string, { id: string; key: string; name: string }>();
