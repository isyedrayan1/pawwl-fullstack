...
`;
export const sendOrderConfirmation = async (to, orderNumber, total) => {
...
export const sendShippingUpdate = async (to, orderNumber, trackingUrl) => {
    if (!env.gmailUser || !env.gmailAppPassword)
        return;
...
export const sendOrderDelivered = async (to, orderNumber) => {
    if (!env.gmailUser || !env.gmailAppPassword)
        return;
    const content = `
...
export const sendOrderCancelled = async (to, orderNumber) => {
    if (!env.gmailUser || !env.gmailAppPassword)
        return;
    const content = `
...
export const sendAbandonedCart = async (to, name) => {
    const html = getBaseEmailTemplate("You left something behind! 🛒", `
...
export const sendNewJobApplicationNotification = async (app) => {
    console.log("Mock sending job app email:", app);
};
export const sendServiceLeadEmail = async (lead) => {
    const adminEmails = ["admin@pawwl.com", "support.pawwl@gmail.com"];
    const html = getBaseEmailTemplate("New Service Booking Request", `
...
export const sendJobApplicationEmail = async (app) => {
    const adminEmails = ["admin@pawwl.com", "support.pawwl@gmail.com"];
    const html = getBaseEmailTemplate("New Job Application Received", `
