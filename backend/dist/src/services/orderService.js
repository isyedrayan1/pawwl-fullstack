...
export const createOrderFromCart = async (userId, addressId, notes, couponCode, isBillingSame = true, billingAddressId) => {
    return prisma.$transaction(async (tx) => {
        ...
        if (!resolvedBillingAddress)
            throw new HttpError(404, "Billing address not found");
        if (items.length === 0)
        ...
        if (issues.length > 0)
            throw new HttpError(400, issues.join("; "));
        let discountValue = 0;
