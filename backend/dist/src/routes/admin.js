...
const logAdminAction = async (adminUserId, action, entityType, entityId, metadata) => {
...
const categoryKey = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
...
const syncDefaultVariant = async (productId, input) => {
...
router.post("/upload", upload.array("images", 10), asyncHandler(async (req, res) => {
...
router.post("/admins", requireSuperAdmin, asyncHandler(async (req, res) => {
    ...
    if (existing)
        throw new HttpError(400, "Username or email already taken");
    const passwordHash = await bcrypt.hash(password, 10);
    const admin = await prisma.user.create({
        data: { id: createId(), name: name ?? username, username, email: email ?? `${username}@pawwl.local`, passwordHash, role, adminRoleId },
        select: adminSelect,
    });
    res.status(201).json({ admin });
}));
router.put("/admins/:id", requireSuperAdmin, asyncHandler(async (req, res) => {
...
router.get("/roles", requireSuperAdmin, asyncHandler(async (_req, res) => {
...
router.post("/roles", requireSuperAdmin, asyncHandler(async (req, res) => {
...
router.put("/roles/:id", requireSuperAdmin, asyncHandler(async (req, res) => {
    const { name, permissions } = roleSchema.parse(req.body);
...
router.delete("/roles/:id", requireSuperAdmin, asyncHandler(async (req, res) => {
    ...
    if (role._count.users > 0)
        throw new HttpError(400, "Cannot delete role assigned to users");
    await prisma.adminrole.delete({ where: { id: String(req.params.id) } });
    res.json({ success: true });
}));
// --- End Admin Roles Management ---
router.get("/summary", requireSuperAdmin, asyncHandler(async (_req, res) => {
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const [usersTotal, activeUsers, disabledUsers, productsTotal, publishedProducts, draftProducts, archivedProducts, lowStockVariants, recentOrders, recentUsers, recentLogs, revenueAggregate, pendingOrders, processingOrders, deliveredOrders, orders7d,] = await Promise.all([
...
router.post("/marketing/abandoned-carts/send", requireSuperAdmin, asyncHandler(async (req, res) => {
...
router.patch("/users/:id/status", requireSuperAdmin, asyncHandler(async (req, res) => {
...
router.delete("/products/:id", asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (error.code === "P2003" || (error.message && error.message.includes("foreign key constraint"))) {
            throw new HttpError(400, "Cannot delete this product because it has order history. Please change its status to 'Archived' instead.");
        }
        throw error;
    }
}));
...
router.post("/coupons", asyncHandler(async (req, res) => {
    ...
    if (existing)
        throw new HttpError(400, "A coupon with this code already exists");
...
router.patch("/returns/:id/status", asyncHandler(async (req, res) => {
...
// EXCEL EXPORT ROUTES
const sendExcel = (res, data, filename) => {
...
// POS Order Creation
router.post("/pos-order", asyncHandler(async (req, res) => {
...
router.get("/leads/services", requireScope("manage_orders"), asyncHandler(async (req, res) => {
    const leads = await prisma.servicelead.findMany({
        orderBy: { createdAt: "desc" },
    });
    res.json({ leads });
}));
router.get("/leads/careers", requireScope("manage_users"), asyncHandler(async (req, res) => {
