...
export type CatalogAnimalType = "Cat" | "Dog" | null;
...
export const parseCatalogCsv = (csvText: string): CatalogSourceRow[] => {
  const rows: string[][] = [];
...
export const discoverCatalogImages = (productsRoot: string, catalogId: number) => {
  const catalogFolder = path.resolve(productsRoot, String(catalogId));
  if (!fs.existsSync(catalogFolder)) return [] as string[];

  const discovered: string[] = [];
  const queue = [catalogFolder];

  while (queue.length > 0) {
    const currentFolder = queue.pop() as string;
...
export const inferAnimalType = (name: string): CatalogAnimalType => {
...
export const inferCategory = (name: string): string => {
  ...
  if (/(bowl|collar|leash|bed|harness|crate|accessor|brush|comb)/i.test(lowerName)) return "Accessories";
  if (/(cat litter|wonder white|lavender|lavendar|baby powder|hygiene|pad|wipes|sanit|soap)/i.test(lowerName)) return "Litter & Hygiene";
  if (/(anti hairball|hairball|supplement)/i.test(lowerName)) return "Supplements";
  if (/(toy|ball|mouse|feather|chaser|play)/i.test(lowerName)) return "Toys";
  if (/(treat|jerky|sticks?|biscuit|biscrok|creamy|lickable|crunchy|snack|dentastix|panna cotta|slice)/i.test(lowerName)) return "Treats";
  if (/(pouch|tin|gravy|jelly|canned|can |chunk in gravy|wet food|medley)/i.test(lowerName)) return "Wet Food";
...
const extractPackSize = (name: string) => {
  const match = name.match(/(?:^|[-\s])(\d+(?:\.\d+)?\s?(?:kg|g|gm|tubes?|sticks?|x\d+))/i);
  return match ? match[1].replace(/\s+/g, " ").trim() : "";
};

const inferLifeStage = (name: string, animalType: CatalogAnimalType) => {
  const lowerName = name.toLowerCase();
  if (lowerName.includes("kitten")) return "kitten";
  if (lowerName.includes("puppy")) return "puppy";
  if (lowerName.includes("junior")) return animalType === "Dog" ? "puppy" : "kitten";
  if (lowerName.includes("adult")) return "adult";
...
const formatLabel = (animalType: CatalogAnimalType) => {
  if (animalType === "Cat") return "cats";
  if (animalType === "Dog") return "dogs";
  return "pets";
};

const formatLifeStageLabel = (lifeStage: string, animalType: CatalogAnimalType) => {
  if (lifeStage === "kitten" || lifeStage === "puppy") return lifeStage;
  if (lifeStage === "adult") return animalType === "Cat" ? "adult cats" : animalType === "Dog" ? "adult dogs" : "adult pets";
  if (animalType === "Cat") return "cats";
  if (animalType === "Dog") return "dogs";
  return "pets";
};
...
const buildBenefits = (category: string, animalType: CatalogAnimalType, name: string) => {
  const lowerName = name.toLowerCase();
...
const buildUsage = (category: string, animalType: CatalogAnimalType, name: string) => {
  const lowerName = name.toLowerCase();
...
export const buildCatalogBackup = (params: {
  ...
  const items = rows.map((row, index) => {
    ...
    const status: CatalogStatus = imagePaths.length > 0 ? "published" : "draft";
  ...
  const animalTypes = Array.from(new Set(items.map((item) => item.animalType).filter((value): value is NonNullable<CatalogAnimalType> => Boolean(value))));
