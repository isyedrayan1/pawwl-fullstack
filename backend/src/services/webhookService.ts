...
import { sendOrderConfirmation, sendOrderCancelled } from "../lib/mailer.js";
...
export const handleRazorpayWebhook = async (
  ...
  if (event === "payment.captured" || event === "order.paid") {
    ...
    if (!providerOrderId) return { ok: true, message: "No order_id in payload" };
  ...
  if (event === "payment.failed") {
    ...
    if (!payment) return { ok: true, message: "Payment not found" };
